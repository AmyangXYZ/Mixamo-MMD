...
export function loadAnimJson(json: Record<string, unknown>): AnimData {
  const container = (json as { namedVariants: { variant: Record<string, unknown> }[] }).namedVariants[1].variant
...
/** World rotation per bone (same FK as positions; frameIdx -1 = reference pose only). */
export function computeWorldRotations(anim: AnimData, frameIdx: number): [number, number, number, number][] {
  const n = anim.bones.length
  const wRot: [number, number, number, number][] = new Array(n)
